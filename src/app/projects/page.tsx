...
export default function Projects() {
  ...
  const list = cat === "All" ? PROJECTS : PROJECTS.filter((p) => p.cats.includes(cat));
            ...
            <article key={p.slug} className="work-item group mb-10">
              <button onClick={() => navigate("project/" + p.slug)} data-cursor="View" className="block w-full text-left">
                <div className="relative">
                  <Img src={p.poster} alt={`${p.title} — ${p.cats.join(", ")}`} ratio={RATIOS[i % RATIOS.length]} drift={7} reveal={false} />
